...
This approach is difficult to implement, as the realation is cyclic.
We propose to use a different solution to this problem: The
Config UTXO. \\
...
scripts. In our case, the Config UTXO will contain the following data in the datum: 
the liquidity token minting policy hash and the pool validator script address. \\

% TODO explain what do we want to decouple from/ motivate why we want to decouple
% the swaps and (what liquidity interactions, all of them?) possible without the config utxo
% The following sentence implies that we actually make use of a hybrid between 
% Config UTXO and the naive approach
...
the Config UTXO sub-graph only includes an arrow from the pool id minting policy
...
We need a way to distinguish and validate the Config UTXO from any
other Config UTXOs. This can be implemented using a boilerplate NFT contract.
