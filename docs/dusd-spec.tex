...
Price module:
\begin{itemize}
  \item The off-chain code will be built in CTL.
  ...
  \item The validator will validate the following:
    \begin{itemize}
      ...
      \item The previous price oracle UTxO contains the Oracle NFT.
      \item The datum has a list of tuples containing USD Price of ADA and the POSIX timestamp 
        of when the price was observed.
      \item The datum list does not exceed 48 entries.
      \item The timestamp of the latest two entries of the datum list are at least an hour apart.
      \item The timestamp of the first entry in the datum list is within an hour of the timestamp
        of the transaction.
      \item The tail of the datum list matches the sequence of the previous price oracle UTxO's 
        datum list starting from it's first element.
      \item The timestamp of the previous price oracle UTxO is at least an hour behind the
        transaction.
      \item The Oracle NFT is being moved to the new Price Oracle UTxO.
    \end{itemize}  
  \end{itemize} 
